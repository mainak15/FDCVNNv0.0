...
def train_model(model, train_loader, val_loader, test_loader, criterion, optimizer, num_epochs=20):
    ...
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=8, factor=0.5) 
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), './Saved_Model/PTB_log_1506RB4.pth')
    ...
    model.load_state_dict(torch.load('./Saved_Model/PTB_log_1506RB4.pth')) #  
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc2 = FrequencyLinear(512, 2)
...
ptbdb_abnormal = pd.read_csv("/home/mainak/MDU/New FCNN/Ablation/data/ECG/ptbdb_abnormal.csv", header=None)
ptbdb_normal = pd.read_csv("/home/mainak/MDU/New FCNN/Ablation/data/ECG/ptbdb_normal.csv", header=None)
...
ptbdb.iloc[:, -1] = ptbdb.iloc[:, -1].astype(int)


X_ptbdb_train, X_ptbdb_val, y_ptbdb_train, y_ptbdb_val = train_test_split(
    ptbdb.iloc[:, :-1].values, ptbdb.iloc[:, -1].values, test_size=0.2, random_state=42)
X_ptbdb_val, X_ptbdb_test, y_ptbdb_val, y_ptbdb_test  = train_test_split(
    X_ptbdb_val, y_ptbdb_val, test_size=0.5, random_state=42)
...
X_ptbdb_train = X_ptbdb_train[:, np.newaxis, :, np.newaxis]
X_ptbdb_val = X_ptbdb_val[:, np.newaxis, :, np.newaxis]
X_ptbdb_test = X_ptbdb_test[:, np.newaxis, :, np.newaxis]
...
trainset_fft = FFTDataset(X_ptbdb_train, y_ptbdbl_train)
valset_fft = FFTDataset(X_ptbdb_val, y_ptbdbl_val)
testset_fft = FFTDataset(X_ptbdb_test, y_ptbdbl_test)
