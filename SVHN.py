...
def train_model(model, train_loader, val_loader, test_loader, criterion, optimizer, num_epochs):
    ...
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=50, factor=0.1)
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss <= best_val_loss:
            ...
            torch.save(model.state_dict(), './Saved_Model/SVHN_LOG_1506RB4.pth')
    ...
    model.load_state_dict(torch.load('./Saved_Model/SVHN_LOG_1506RB4.pth'))
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = FrequencyConv2D(3, 64, kernel_size=32)
        self.bn1 = FrequencyInstanceNorm2D(64)
        
        self.layer1 = self._make_layer(64, 64, kernel_size=16, num_blocks=1)
        self.layer2 = self._make_layer(64, 128, kernel_size=8, num_blocks=1)
        self.layer3 = self._make_layer(128, 256, kernel_size=4, num_blocks=1)
...
train_size = int(0.92 * len(trainset))
...
trainset_fft = FFTDataset(train_set)
valset_fft = FFTDataset(val_set)
testset_fft = FFTDataset(testset)
...
train_losses, val_losses, train_accuracies, val_accuracies = train_model(model, trainloader_fft, validationloader_fft, testloader_fft, criterion, optimizer, num_epochs=5000)
